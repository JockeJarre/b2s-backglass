...
- **B2SServerPluginInterface target**: The upstream project still targets .NET Framework 4.0; install the .NET 4.0 Developer Pack (e.g., `choco install netfx-4.0.3-devpack -y`) or restore `Microsoft.NETFramework.ReferenceAssemblies.net40` and pass `/p:FrameworkPathOverride` before building it locally
